...
def _make_pbr_antitile_uvb_fac_group(group_name="NG_PBR_AntiTile_UVB_Fac_FromDualNoise"):
    """
    ...
    add_socket(ng, in_out="INPUT", name="Warped Vector", socket_type="NodeSocketVector", default=(0.0, 0.0, 0.0))
    add_socket(ng, in_out="INPUT", name="AntiTile Noise Centered", socket_type="NodeSocketFloat", default=0.0)
    add_socket(ng, in_out="INPUT", name="AntiTile Enable", socket_type="NodeSocketFloat", default=0.0, min_val=0.0, max_val=1.0)
    add_socket(ng, in_out="INPUT", name="AntiTile Blend", socket_type="NodeSocketFloat", default=1.0, min_val=0.0, max_val=1.0)
    add_socket(ng, in_out="INPUT", name="AntiTile Angle", socket_type="NodeSocketFloat", default=math.radians(60.0), min_val=-math.pi, max_val=math.pi)
    add_socket(ng, in_out="INPUT", name="AntiTile Offset", socket_type="NodeSocketVector", default=(0.37, 0.11, 0.0))
    ...
    rot = nodes.new("ShaderNodeCombineXYZ"); rot.location = (-460, -120)
    links.new(gin.outputs["AntiTile Angle"], rot.inputs["Z"])

    mapping = nodes.new("ShaderNodeMapping"); mapping.location = (-220, -40)
...
def ensure_pbr_antitile_uvb_fac_group():
    ...
    ng = get_or_create_group("NG_PBR_AntiTile_UVB_Fac_FromDualNoise", _build)
    return rebuild_group_if_missing_inputs("NG_PBR_AntiTile_UVB_Fac_FromDualNoise", required, _build)
